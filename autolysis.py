...
def define_analysis_tools():
    return [
        ...
        },
        {
            ...
            "description": "Perform geographic clustering based on latitude and longitude.",
            "parameters": {
                "type": "object",
                "properties": {
                    "latitude": {
...
def request_llm_storytelling(info, analysis_results, chart_paths):
    # Generates a narrative about the dataset, analysis, and charts using LLM.

    # Parameters:
    #     info (dict): Information about the dataset.
    #     analysis_results (dict): Results of the analysis performed on the dataset.
    #     chart_paths (list): A list of image filenames for the charts.

    # Returns:
    #     str: Narrative generated by the LLM, or None if an error occurs.
...
def visualize_outliers(outliers):
    # Visualizes a list of outliers using a stripplot and saves the visualization as an image.

    # Parameters:
    #     outliers (list): A list of numerical outliers to visualize.
...
def visualize_correlation_matrix(correlation_matrix):
    ...
    # Visualizes a correlation matrix using a heatmap and returns the figure object for further processing.
    
    # Parameters:
    # correlation_matrix (2D array-like): A correlation matrix to visualize.
    
    # Returns:
    # fig: The matplotlib figure object for further manipulation or saving.
    
    # Convert the correlation matrix to a pandas DataFrame if it is not already
...
def visualize_geographical_analysis(geo_data):
    # Visualizes geographical data using a scatterplot and returns the figure object for further processing.

    # Parameters:
    #     geo_data (list of tuples): A list of tuples containing latitude and longitude pairs.

    # Returns:
    #     fig: The matplotlib figure object for further manipulation or saving.
    
    # Extract latitude and longitude
    latitudes, longitudes = zip(*geo_data)

    # Create the figure and axis
    fig, ax = plt.subplots(figsize=(6.4, 6.4))  # Original size to match the 512x512 px requirement after saving

    # Generate the scatterplot
    ax.scatter(longitudes, latitudes, color="blue", alpha=0.7)

    # Add titles and labels
    ax.set_title("Geographical Analysis", fontsize=14)
...
def visualize_time_series(time_series):
    # Visualizes time series data using a line plot and returns the figure object for further processing.

    # Parameters:
    #     time_series (list of tuples): A list of tuples containing time and value pairs.
    ...
    # Add titles and labels
    ax.set_title("Time Series Analysis", fontsize=14)
...
def create_directory_for_file(file_name):
    # Create directory based on the file name (without extension)
        ...
        for tool in tools:
            ...
            if relevant_data:
                ...
                if result:
                    ...
                    # Now Visulaizing the outliers and correlation
                    ...
                    elif tool["name"] == "time_series_analysis":
                        chart = visualize_time_series(result)
                        chart_paths.append(save_chart(chart, directory_name, "timeseries.png"))
                    else:
                        chart = visualize_geographical_analysis(result)
                        chart_paths.append(save_chart(chart, directory_name, "geography.png"))
